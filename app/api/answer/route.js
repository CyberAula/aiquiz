...
export async function POST(request) { 
    try {
        const { id, subject, topic, difficulty, subTopic, query, choices, answer, correct, explanation, studentEmail, studentAnswer, studentReport, llmModel, ABC_Testing, md5Prompt, prompt, pull_id, usageData, responseTime, studentResponseTime, modelId, modelConfig, numQuestionsRequested, numQuestionsReceived, estimatedCost } = await request.json();
        ...
        if(questions.length>0){
            ...
            const newQuestion = new Question({ id, subject, topic, difficulty, subTopic, query, choices, answer, explanation, studentEmail, studentAnswer, correct, studentReport, llmModel, ABC_Testing, md5Prompt, prompt, pull_id, usageData, responseTime, studentResponseTime, modelId, modelConfig, numQuestionsRequested, numQuestionsReceived, estimatedCost });
