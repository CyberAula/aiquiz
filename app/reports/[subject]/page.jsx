  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const responseEvaluatedReported = await fetch(urljoin(basePath,`/api/reports?studentReport=true&&asignatura=${subject}&&count=true&&evaluadas=true&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
        ...
        let comments = getSpanishComments();
            ...
            await Promise.all(
              comments.map(async (reason) => {
                const responseReason = await fetch(urljoin(basePath,`/api/reports?studentReport=true&&asignatura=${subject}&&count=true&&evaluadas=true&&motivo=${reason}&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
        ...
        let difficulties = ["facil", "intermedio", "avanzado"];
            ...
            await Promise.all(
              difficulties.map(async (difficulty) => {
                const responseDifficultyFrequency = await fetch(urljoin(basePath, `/api/reports?dificultad=${difficulty}&&asignatura=${subject}&&count=true&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
                const nDifficultyFrequency = (await responseDifficultyFrequency.json()).count;

                const responseDifficultySuccess = await fetch(urljoin(basePath, `/api/reports?dificultad=${difficulty}&&count=true&&asignatura=${subject}&&acierto=true&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
          ...
          subjectTopics = subjects[subject].topics.map(topic => topic.label);
            ...
            await Promise.all(
              subjectTopics.map(async (topic) => {
                ...
                const responseTopicFrequency = await fetch(urljoin(basePath, `/api/reports?tema=${topic}&&asignatura=${subject}&&count=true&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
                const nTopicFrequency = (await responseTopicFrequency.json()).count;

                const responseTopicSuccess = await fetch(urljoin(basePath, `/api/reports?tema=${topic}&&asignatura=${subject}&&count=true&&acierto=true&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
                  ...
                  if (topic.subtopics) {
                    await Promise.all(
                      topic.subtopics.map(async (sub) => {
                        ...
                        const responseSubtopicFrequency = await fetch(urljoin(basePath, `/api/reports?subtema=${subtopic.toLowerCase()}&&count=true&&asignatura=${subject}&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
                        const nSubtopicFrequency = (await responseSubtopicFrequency.json()).count;

                        const responseSubtopicSuccess = await fetch(urljoin(basePath, `/api/reports?subtema=${subtopic.toLowerCase()}&&count=true&&acierto=true&&asignatura=${subject}&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
        ...
        if (startDate1!=null) {
            ...
            if (diffMonths <= 3) { // If range is 3 months or less, show and query by days
              ...
              while (current <= end) {
                ...
                try {
                  const response = await fetch(urljoin(basePath, `/api/reports?temporal=true&&count=true&&dia=${day}&&mes=${month}&&anio=${year}&&asignatura=${subject}&&fechaInicio=${startDate1}&&fechaFin=${endDate1}`));
                    if (!response.ok)
   ...
   useEffect(() => {
    ...
    const fetchComparator = async () => {
       ...
       try {
        ...
        // Fetch EVALUATED reported questions
        const responseEvaluatedReported = await fetch(urljoin(basePath,`/api/reports?studentReport=true&&asignatura=${subject}&&count=true&&evaluadas=true&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
        if (!responseEvaluatedReported.ok)
          throw new Error("Error loading reported questions");
        const resultEvaluatedReported = (await responseEvaluatedReported.json()).count;
        ...
        let comments = getSpanishComments();
            ...
            await Promise.all(
              comments.map(async (reason) => {
                const responseReason = await fetch(urljoin(basePath,`/api/reports?studentReport=true&&asignatura=${subject}&&count=true&&evaluadas=true&&motivo=${reason}&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                const nReason = (await responseReason.json()).count;
        ...
        let difficulties = ["facil", "intermedio", "avanzado"];
            ...
            await Promise.all(
              difficulties.map(async (difficulty) => {
                const responseDifficultyFrequency = await fetch(urljoin(basePath, `/api/reports?dificultad=${difficulty}&&asignatura=${subject}&&count=true&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                const nDifficultyFrequency = (await responseDifficultyFrequency.json()).count;

                const responseDifficultySuccess = await fetch(urljoin(basePath, `/api/reports?dificultad=${difficulty}&&count=true&&asignatura=${subject}&&acierto=true&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                const nDifficultySuccess = (await responseDifficultySuccess.json()).count;
          ...
          subjectTopics = subjects[subject].topics.map(topic => topic.label);
            ...
            await Promise.all(
              subjectTopics.map(async (topic) => {
                ...
                const responseTopicFrequency = await fetch(urljoin(basePath, `/api/reports?tema=${topic}&&asignatura=${subject}&&count=true&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                const nTopicFrequency = (await responseTopicFrequency.json()).count;

                const responseTopicSuccess = await fetch(urljoin(basePath, `/api/reports?tema=${topic}&&asignatura=${subject}&&count=true&&acierto=true&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                const nTopicSuccess = (await responseTopicSuccess.json()).count;
        ...
        if (startDate2!=null) {
            ...
            if (diffMonths <= 3) {
              ...
              while (current <= end) {
                ...
                try {
                  // Query by day
                  const response = await fetch(urljoin(basePath, `/api/reports?temporal=true&&count=true&&dia=${day}&&mes=${month}&&anio=${year}&&asignatura=${subject}&&fechaInicio=${startDate2}&&fechaFin=${endDate2}`));
                    if (!response.ok)
                    throw new Error("Error loading daily usage data");
