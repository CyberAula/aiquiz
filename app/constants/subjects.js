export const subjects = {
        ...
        topics: [
            {
                ...
                subtopics: [ 
                    ...
                    { title: 'URLs', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> Una URL (Localizador Uniforme de Recursos) es la dirección que permite acceder a un recurso en Internet. Es un caso específico de los URI (Identificadores Uniformes de Recursos) y fue desarrollada como parte fundamental de la Web. El formato estándar de una URL es: "scheme://user:password@host:port/path?query#fragment" 
                        ...
                        Los navegadores realizan este proceso automáticamente, pero en otras aplicaciones debe hacerlo el desarrollador manualmente.</resumen>`, files: [] },
                    { title: 'HTTP y Cabeceras', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> HTTP (Hypertext Transfer Protocol) es un protocolo de comunicación sin estado basado en un modelo de petición-respuesta entre cliente y servidor. Toda transacción se inicia desde el cliente (normalmente un navegador), que envía una solicitud HTTP y recibe una respuesta HTTP del servidor. Ambas tienen una estructura común: una cabecera y, opcionalmente, un cuerpo (body).
                        ...
                        HTTP ha evolucionado desde la versión 1.0 hasta HTTP/2 y HTTP/3, que mejoran la eficiencia con conexiones persistentes, compresión de cabeceras y uso de QUIC sobre UDP.</resumen>`, files: [] },
                    { title: 'Interfaz HTTP, REST, Estado y Carga de Páginas', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> El interfaz HTTP permite que el cliente acceda a servicios del servidor mediante rutas (URLs) y métodos HTTP (GET, POST, PUT, DELETE). Se distingue entre:
                        ...
                        Finalmente, al cargar una página web, el navegador puede establecer múltiples conexiones para obtener recursos adicionales (CSS, imágenes, etc.), incluso desde diferentes servidores.</resumen>`, files: [] }
                ]
            },
            {
            ...
            },
            {
                ...
                subtopics: [
                    ...
                    { title: 'Arrays y objetos', 
                      comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> El índice de inicio es inclusivo y la cantidad es el número de elementos a eliminar.
                ...
                subtopics: [

                    {
                        ...
                        Se explica qué es un ORM (Object Relational Mapping):
                          ...
                          - Es un sustituto en programación para no usar directamente SQL. </resumen>`,
            ...
            },
            {
                ...
                subtopics: [
                    { title: 'Estructura proyecto express', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> - El fichero bin/www es el punto de entrada de un servidor en express. En el se crea el servidor y empieza a escuchar en el puerto configurado. 
                        ...
                        - El fichero package.json incluye la información, dependencias, y scripts del proyecto. </resumen>`, files: [] },
                    { title: 'Comandos inicio proyecto express', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen>  Para iniciar un proyecto de express desde cero hay que ejecutar los siguientes comandos:
                        ...
                        - npm install
                        - npm start 
                        Se puede instalar el módulo supervisor (npm install supervisor) y configurarlo en package.json para no tener que rearrancar el servidor para que los cambios sean visibles.</resumen>`, files: [] },
                    { title: 'Favicon, layout y consentimiento de cookies', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> Favicon: El favicon es el icono de nuestra aplicación web que aparece en la pestaña del navegador. Para añadirlo es necesario instalar el módulo serve-favicon y configurarlo en app.js. La imagen con el favicon debe guardarse en la carpeta public.
                        ...
                        Consentimiento de cookies: La ley obliga a pedir consentimiento al usuario cuando se usan cookies. Para ello debemos de instalar el módulo cookieconsent y configurarlo en las vistas para que al usuario le aparezca el popup advirtiendo del uso de cookies</resumen>`, files: [] }
                ]
            },
            {
                ...
                subtopics: [
                    ...
                    { title: 'Mehtod-override', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> method-override es un middleware de Express que permite usar métodos HTTP como PUT, PATCH y DELETE en entornos donde solo se pueden enviar GET y POST, como en formularios HTML. Dado que los formularios solo soportan GET y POST, method-override intercepta las solicitudes y reemplaza el método original por el que especifiques, permitiendo así seguir una arquitectura RESTful incluso con formularios.
                        ...
                        Ejemplo: action="/recurso/1?_method=DELETE" method="POST" enviará un POST pero el servidor procesará la solicitud como si fuera un DELETE y así será procesado rutas 'router.delete' y no por 'router.post'</resumen>`, files: [] },
                    { title: 'Primer CRUD', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> Para implementar un CRUD (CREATE, READ, UPDATE, DELETE) básico en Express debemos seguir los siguientes pasos:
                        ...
                        5. Desarrollar la vista de respuesta para el usuario, la redirección, o la respuesta HTTP específica en cada caso.</resumen>`, files: [] },
                    { title: 'Autoload', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> En rutas que acceden a un parámetro, como en el miniproyecto :quizId lo habitual es implementar un middleware de autoload. El middleware de autoload lo que hace es en las peticiones que contienen el param :quizId buscar el quiz con ese id en la base de datos, guardarlo en la variable req.load.quiz y propagar la petición a los siguientes middlewares ejecutando next() que tendrán disponible el quiz ya cargado desde la base de datos. El autoload ayuda a organizar el código.</resumen>`, files: [] },
                    { title: 'Subir ficheros', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> 
                        ...
                        3. Almacenamiento: Los ficheros subidos se pueden guardar en el sistema de ficheros, por ejemplo en la carpeta public si van a ser accesibles como estáticos. Otra opción es guardarlos en la base de datos. Para ello se puede utilizar el tipo de datos Sequelize.BLOB (Binary Large Objects)</resumen>`, files: [] },
            ...
            },
            {
                ...
                subtopics: [
                    ...
                    { title: 'Mensajes flash', comment: `Este es el resumen de la materia desarrollada en clase que puede servirte para orientar el contenido de las preguntas de este tema, aunque pueden ser más amplias y puedes utilizar conocimientos generales relacionados: <resumen> Los mensajes de Flash son mensajes que el servidor guarda en la sesión mientras atiende las peticiones HTTP recibidas de un cliente, y que mostrará en la próxima vista generada. Su uso surge cuando al procesar una petición HTTP del cliente hay redirecciones pues los mensajes informativos generados en la peticiones HTTP anteriores se pierden dado que HTTP no tiene estado. Solución usar mensajes flash: 1. Guardar estos mensajes en la sesión (req.session) para que no se pierdan, y recuperarlos en la siguiente respuesta HTTP que envíe una vista al cliente. 2. La vista mostrará todos los mensajes pendientes almacenados. Módulo express-flash configurado en app.js. Las vistas puedan acceder a los mensajes de flash a través de res.locals.messages. Los mensajes de flash se crean y se guardan en la sesión agrupados por tipos usando la llamada: req.flash(tipo, msg)
                        ...
                        Es necesario modificar las vistas si se quieren pintar los mensajes flash.</resumen>`, files: [] },
                ]
            },
            {
    ...
    BDNR: {
        name: 'Bases de Datos No Relacionales y distribuidas',
        topics: [
            {
                value: 'intro_nosql', label: 'Introducción a NoSQL',
                subtopics: [
    ...
    BDFI: {
        name: 'Big Data Fundamentos e Infraestructura',
        topics: [
            {
                value: 'intro_nosql', label: 'Introducción a NoSQL',
                subtopics: [
        ...
        topics: [
            {
                ...
                subtopics: [
                    { title: 'Diseño y configuración de página y tipos', comment: `Las preguntas de este tema pueden incluir configuraciones de márgenes, orientación (vertical/horizontal), tamaño de papel, y tipos de documento en procesadores de texto como Microsoft Word. <resumen>Configurar una página implica definir márgenes, orientación, tamaño del papel y secciones del documento. Los tipos de documento pueden ser cartas, informes, folletos, etc., cada uno con configuraciones específicas.</resumen>`, files: [] },
                    { title: 'Columnas', comment: `Las preguntas de este tema evalúan el uso de columnas para organizar el texto en un documento, incluyendo número de columnas, espaciado y líneas divisorias. <resumen>Las columnas permiten dividir el texto en secciones verticales dentro de una página, útil para boletines y periódicos. Se pueden configurar el número de columnas, el ancho, el espaciado entre ellas y agregar líneas divisorias.</resumen>`, files: [] },
                    { title: 'Aplicación de Estilos', comment: `Las preguntas de este tema evalúan el uso de estilos predefinidos y personalizados para aplicar formato consistente a títulos, párrafos y otros elementos del documento. <resumen>Los estilos son conjuntos de formatos (fuente, tamaño, color, interlineado, etc.) que se aplican de forma consistente a diferentes partes del documento. Permiten mantener uniformidad y facilitan la generación de tablas de contenido automáticas.</resumen>`, files: [] },
                    { title: 'Inserción de objetos', comment: `Las preguntas de este tema pueden abarcar la inserción de imágenes, formas, gráficos en un documento. <resumen>En un procesador de texto se pueden insertar diversos objetos como imágenes (desde archivo o en línea), formas geométricas, SmartArt.</resumen>`, files: [] },
                    { title: 'Combinación de correspondencia', comment: `Las preguntas de este tema evalúan el proceso de combinar un documento principal con una fuente de datos para generar documentos personalizados en masa. <resumen>La combinación de correspondencia permite crear múltiples documentos personalizados (cartas, etiquetas, correos) a partir de un documento base y una lista de datos (Excel, Access, etc.). El proceso incluye: seleccionar el tipo de documento, vincular la fuente de datos, insertar campos de combinación y ejecutar la combinación.</resumen>`, files: [] },
                    { title: 'Creación de sobres y etiquetas', comment: `Las preguntas de este tema evalúan la configuración y generación de sobres y etiquetas postales mediante combinación de correspondencia. <resumen>Word permite crear sobres y etiquetas configurando el tamaño, la dirección del destinatario y del remitente en masa usando una lista de datos, definiendo el tipo de etiqueta comercial o un tamaño personalizado.</resumen>`, files: [] },
                    { title: 'Formularios', comment: `Las preguntas de este tema evalúan la creación de formularios con controles interactivos (campos de texto, listas desplegables, casillas de verificación) usando la pestaña Desarrollador. <resumen>Los formularios en Word utilizan controles de contenido (texto, fecha, lista desplegable, casilla de verificación, etc.) disponibles en la pestaña Desarrollador. Se pueden proteger para que el usuario solo complete los campos habilitados, y se usan para recopilar información de forma estructurada.</resumen>`, files: [] }
                ]
            },

            {
                value: 'manejo_texto', label: 'Manejo de Texto',
                subtopics: [
                    { title: 'Saltos de página y secciones', comment: `Las preguntas de este tema evalúan el uso de saltos de página y secciones para controlar el flujo y el formato independiente de distintas partes del documento. <resumen>Los saltos de página fuerzan el inicio de contenido en una nueva página. Los saltos de sección permiten aplicar formatos distintos (orientación, columnas, encabezados, etc.) a diferentes partes del documento. Tipos de sección: página siguiente, continuo, página par e impar.</resumen>`, files: [] },
                    { title: 'Búsqueda y reemplazo', comment: `Las preguntas de este tema evalúan el uso de las herramientas Buscar y Reemplazar para localizar y sustituir texto, formatos o caracteres especiales en el documento. <resumen>La función Buscar (Ctrl+F) permite localizar texto en el documento. Reemplazar (Ctrl+H) permite sustituirlo por otro texto o formato. Ambas admiten opciones avanzadas como coincidencia exacta, uso de comodines y búsqueda de formato específico.</resumen>`, files: [] },
                    { title: 'Encabezados y pies de página', comment: `Las preguntas de este tema evalúan la inserción y configuración de encabezados y pies de página, incluyendo numeración, diferenciación por sección y primera página distinta. <resumen>Los encabezados y pies de página son áreas en la parte superior e inferior de cada página que pueden contener texto, números de página, fecha, logotipos, etc. Se pueden configurar de forma diferente para la primera página, páginas pares e impares, o por secciones del documento.</resumen>`, files: [] },
                    { title: 'Operaciones de creación y manejo de tablas', comment: `Las preguntas de este tema evalúan la inserción, edición y formato de tablas, incluyendo combinación de celdas, ordenación de datos y conversión texto-tabla. <resumen>Las tablas en Word permiten organizar información en filas y columnas. Se pueden insertar, eliminar y combinar celdas, aplicar estilos de tabla, ordenar el contenido, ajustar el tamaño de filas y columnas, y convertir texto tabulado en tabla y viceversa.</resumen>`, files: [] },
                    { title: 'Marcadores e Hipervínculos', comment: `Las preguntas de este tema evalúan la creación de marcadores para señalar posiciones en el documento y el uso de hipervínculos para navegar hacia URLs, archivos, correos o secciones internas. <resumen>Los marcadores identifican una posición o selección de texto en el documento para referencia futura. Los hipervínculos permiten enlazar texto o imágenes a páginas web, archivos, direcciones de correo electrónico, u otras partes del mismo documento (usando marcadores).</resumen>`, files: [] },
                    { title: 'Ortografía', comment: `Las preguntas de este tema evalúan el uso de las herramientas de revisión ortográfica y gramatical, incluyendo el corrector automático y el diccionario personalizado. <resumen>Word incluye un corrector ortográfico y gramatical que subraya los errores en rojo (ortografía) y azul o verde (gramática). Se puede revisar el documento completo, agregar palabras al diccionario personalizado, configurar la autocorrección y establecer el idioma de revisión.</resumen>`, files: [] },
                    { title: 'Revisión, control de cambios, restricciones de edición y comentarios', comment: `Las preguntas de este tema evalúan el uso colaborativo del documento, incluyendo el seguimiento de modificaciones, la inserción de comentarios y la protección del documento contra ediciones no autorizadas. <resumen>El control de cambios registra todas las modificaciones realizadas en el documento (inserciones, eliminaciones, formato). Los comentarios permiten anotar el documento sin modificar el contenido. Las restricciones de edición limitan qué partes del documento pueden ser modificadas y por quién.</resumen>`, files: [] },
                    { title: 'Protección de documento', comment: `Las preguntas de este tema evalúan los métodos para proteger un documento con contraseña, restringir la edición o el formato, y marcar el documento como final. <resumen>Word permite proteger documentos mediante contraseña de apertura o de escritura, restringir el tipo de edición permitida (solo comentarios, solo rellenar formularios, sin cambios), y cifrar el documento.</resumen>`, files: [] }
                ]
            },

            {
                ...
                subtopics: [
                    { title: 'Insertar título', comment: `Las preguntas de este tema evalúan la inserción de títulos o epígrafes automáticos para figuras, tablas, ecuaciones y otros objetos, con numeración automática. <resumen>La función Insertar Título en Word permite agregar rótulos numerados automáticamente a figuras, tablas, ecuaciones, etc. Estos títulos pueden ser actualizados automáticamente y se usan como base para generar tablas de ilustraciones.</resumen>`, files: [] },
                    { title: 'Tablas de contenidos', comment: `Las preguntas de este tema evalúan la generación automática de tablas de contenido (índices) basadas en los estilos de título aplicados en el documento, así como su actualización. <resumen>La tabla de contenido se genera automáticamente a partir de los párrafos con estilos de título (Título 1, Título 2, etc.). Se puede personalizar el número de niveles, el formato y los caracteres de relleno. Se actualiza con un clic derecho o desde la pestaña Referencias.</resumen>`, files: [] },
                    { title: 'Insertar citas bibliográficas', comment: `Las preguntas de este tema evalúan la gestión de fuentes bibliográficas y la inserción de citas en el texto, con generación automática de la bibliografía al final del documento. <resumen>Word permite administrar una lista de fuentes (libros, artículos, sitios web, etc.) e insertar citas en el texto con formato automático. Al final del documento se puede generar la bibliografía completa. Soporta varios estilos de citación, incluyendo APA.</resumen>`, files: [] },
                    { title: 'Índices alfabéticos', comment: `Las preguntas de este tema evalúan la marcación de entradas de índice y la generación automática del índice alfabético al final del documento. <resumen>Para crear un índice se deben marcar las entradas (palabras clave) usando la opción Marcar entrada en la pestaña Referencias. Word genera el índice alfabético con las páginas donde aparece cada término. Se puede actualizar cuando se modifica el documento.</resumen>`, files: [] },
                    { title: 'Insertar tabla de ilustraciones', comment: `Las preguntas de este tema evalúan la generación automática de una tabla que lista todas las figuras, tablas o ecuaciones con sus títulos y números de página. <resumen>La tabla de ilustraciones se genera a partir de los títulos insertados a figuras, tablas, ecuaciones, etc. Es similar a la tabla de contenido pero específica para objetos. Se actualiza automáticamente cuando se agregan o eliminan objetos con título.</resumen>`, files: [] },
                    { title: 'Referencias cruzadas', comment: `Las preguntas de este tema evalúan la inserción de referencias cruzadas que apuntan a otros elementos del documento (títulos, figuras, tablas, marcadores, notas al pie). <resumen>Las referencias cruzadas permiten hacer alusión a otros elementos del mismo documento (por ejemplo: "Ver Figura 3" o "Consultar la sección 2.1"). Se vinculan dinámicamente y se actualizan si el número o posición del elemento referenciado cambia.</resumen>`, files: [] },
                    { title: 'Uso de normas APA séptima edición', comment: `Las preguntas de este tema evalúan la correcta aplicación del estilo APA séptima edición para la citación en texto y la elaboración de la lista de referencias bibliográficas. <resumen>Las normas APA séptima establecen el formato para citar fuentes dentro del texto (autor, año) y para estructurar la lista de referencias al final del documento. Incluye reglas para para elaborar todo el documento desde la portada del estudiante, niveles en los títulos, formato de figuras, tablas todo sobre la estructuda del documento incluido citas bibliográficas.</resumen>`, files: [] } 
                ]
            },

            {
                value: 'diseno_presentaciones', label: 'Diseño de Presentaciones',
                subtopics: [
                    { title: 'Proceso de planificación y creación de presentaciones', comment: `Las preguntas de este tema evalúan las etapas para planificar y estructurar una presentación efectiva en PowerPoint, incluyendo el esquema de contenido y la audiencia objetivo. <resumen>Una presentación efectiva comienza con la definición del objetivo y la audiencia. Luego se estructura el contenido en un esquema lógico, se selecciona el diseño y tema adecuado, y se distribuye la información de forma clara entre las diapositivas, evitando la sobrecarga de texto.</resumen>`, files: [] },
                    { title: 'Diseño de diapositivas', comment: `Las preguntas de este tema evalúan el uso de temas, plantillas, patrones de diapositivas y la aplicación de fondos, colores y tipografías para lograr un diseño consistente y profesional. <resumen>PowerPoint ofrece temas y plantillas prediseñadas que aplican colores, fuentes y efectos de forma coherente. El patrón de diapositivas permite modificar el diseño base para todas las diapositivas a la vez. Se pueden personalizar fondos, diseños de contenido y elementos gráficos de cada diapositiva.</resumen>`, files: [] },
                    { title: 'Inserción de objetos', comment: `Las preguntas de este tema evalúan la inserción de imágenes, vídeos, audio, gráficos, tablas, SmartArt y otros objetos multimedia en las diapositivas. <resumen>En PowerPoint se pueden insertar imágenes (locales o en línea), formas, iconos, SmartArt, gráficos de datos, tablas, vídeos (locales o de YouTube), archivos de audio, y objetos OLE. Cada objeto puede ser redimensionado, reposicionado y formateado individualmente.</resumen>`, files: [] },
                    { title: 'Animaciones de objetos', comment: `Las preguntas de este tema evalúan la aplicación y configuración de animaciones a objetos dentro de una diapositiva, incluyendo efectos de entrada, salida, énfasis y trayectorias. <resumen>Las animaciones en PowerPoint controlan cómo aparecen, se destacan o desaparecen los objetos dentro de una diapositiva. Se clasifican en: Entrada (aparición), Énfasis (resalte), Salida (desaparición) y Trayectorias de desplazamiento. Se puede configurar el inicio (al hacer clic, con el anterior, después del anterior), la duración y el retardo de cada animación.</resumen>`, files: [] },
                    { title: 'Transiciones entre diapositivas', comment: `Las preguntas de este tema evalúan la aplicación de efectos de transición entre diapositivas, incluyendo la configuración del tiempo, el sonido y el avance automático. <resumen>Las transiciones son efectos visuales que se producen al pasar de una diapositiva a la siguiente. PowerPoint ofrece transiciones sutiles, llamativas y de contenido dinámico. Se puede configurar la duración del efecto, agregar un sonido y establecer si el avance es manual (clic) o automático (tiempo definido).</resumen>`, files: [] }
                ]
            },

            {
                value: 'fundamentos_hoja_calculo', label: 'Fundamentos de Hoja de Cálculo',
                subtopics: [
                    { title: 'Conceptos básicos de los componentes de la hoja de cálculo', comment: `Las preguntas de este tema evalúan el conocimiento de la interfaz de Excel y sus elementos fundamentales: libros, hojas, celdas, rangos, filas, columnas y tipos de datos. <resumen>Una hoja de cálculo (Excel) se organiza en libros compuestos por hojas. Cada hoja es una cuadrícula de filas (números) y columnas (letras). La intersección es una celda, identificada por su referencia (ej. A1). Las celdas pueden contener: texto, números, fechas, valores lógicos o fórmulas. Los rangos son conjuntos de celdas contiguas (ej. A1:C5).</resumen>`, files: [] },
                    { title: 'Funciones lógicas', comment: `El contenido de las preguntas de este tema aplica las siguientes funciones de Microsoft Excel: <resumen>Y, O, NO. La función Y devuelve VERDADERO si todos sus argumentos son verdaderos. La función O devuelve VERDADERO si al menos uno de sus argumentos es verdadero. La función NO invierte el valor lógico de su argumento.</resumen>`, files: [] },
                    { title: 'Función condicional simple y anidada', comment: `Las preguntas de este tema evalúan el uso de la función SI para tomar decisiones basadas en condiciones, incluyendo funciones SI anidadas para múltiples condiciones. <resumen>La función SI evalúa una condición y devuelve un valor si es VERDADERA y otro si es FALSA: =SI(condición, valor_si_verdadero, valor_si_falso). Las funciones SI anidadas permiten evaluar múltiples condiciones encadenando funciones SI como argumentos de otras funciones SI. También se puede combinar con Y, O y NO para condiciones compuestas.</resumen>`, files: [] },
                    { title: 'Validación de datos', comment: `Las preguntas de este tema evalúan la configuración de reglas de validación en celdas para restringir el tipo o rango de datos que el usuario puede ingresar, incluyendo mensajes de entrada y alertas de error. <resumen>La validación de datos en Excel permite restringir los valores permitidos en una celda: números enteros o decimales en un rango, fechas, longitud de texto, o una lista desplegable de valores. Se pueden configurar mensajes de ayuda (al seleccionar la celda) y alertas de error (al ingresar datos no válidos).</resumen>`, files: [] },
                    { title: 'Formato condicional y administración de reglas', comment: `Las preguntas de este tema evalúan la aplicación de formatos automáticos a celdas según condiciones definidas, y la gestión de múltiples reglas de formato condicional. <resumen>El formato condicional aplica automáticamente formatos (color de relleno, color de fuente, bordes, etc.) a las celdas que cumplen una condición específica. Se pueden usar reglas predefinidas (resaltar duplicados, escalas de color, barras de datos) o reglas personalizadas con fórmulas. El administrador de reglas permite crear, editar, eliminar y establecer la prioridad entre varias reglas.</resumen>`, files: [] }
                ]
            },

            {
                value: 'tablas_graficos_dinamicos', label: 'Tablas y Gráficos Dinámicos',
                subtopics: [
                    { title: 'Tablas dinámicas', comment: `Las preguntas de este tema evalúan la creación de tablas dinámicas a partir de un rango de datos para resumir, analizar y explorar información de forma interactiva. <resumen>Una tabla dinámica es una herramienta de Excel que permite resumir grandes volúmenes de datos de forma interactiva. Se crea seleccionando un rango de datos e insertando la tabla dinámica. Los campos se distribuyen en áreas: Filas, Columnas, Valores y Filtros, y pueden reorganizarse fácilmente arrastrando y soltando.</resumen>`, files: [] },
                    { title: 'Cambios en una tabla dinámica', comment: `Las preguntas de este tema evalúan la modificación de la estructura de una tabla dinámica, incluyendo agregar o quitar campos, cambiar el diseño y actualizar los datos de origen. <resumen>Una tabla dinámica puede modificarse en cualquier momento: se pueden agregar, eliminar o mover campos entre las áreas (Filas, Columnas, Valores, Filtros). También se puede cambiar el diseño del informe, actualizar los datos cuando la fuente cambia, y cambiar el rango de datos de origen.</resumen>`, files: [] },
                    { title: 'Filtrado de información', comment: `Las preguntas de este tema evalúan el uso de filtros en tablas dinámicas para mostrar subconjuntos de datos, incluyendo segmentaciones de datos y escalas de tiempo. <resumen>Las tablas dinámicas permiten filtrar los datos usando el área de Filtros, los filtros integrados en etiquetas de fila/columna, las Segmentaciones de datos (slicers) y las Escalas de tiempo (para campos de fecha). Las segmentaciones ofrecen una interfaz visual interactiva para filtrar los datos rápidamente.</resumen>`, files: [] },
                    { title: 'Opciones de tabla', comment: `Las preguntas de este tema evalúan las opciones de configuración de la tabla dinámica, como el nombre, el diseño, los totales y la visualización de errores o celdas vacías. <resumen>Las opciones de tabla dinámica permiten configurar: el nombre de la tabla, el diseño y formato (mostrar totales generales, diseño compacto/esquema/tabular), cómo se muestran los errores y las celdas vacías, y el comportamiento al actualizar. Se accede desde el menú contextual o la pestaña Analizar.</resumen>`, files: [] },
                    { title: 'Configuración de campos', comment: `Las preguntas de este tema evalúan la configuración de los campos de valor en una tabla dinámica, incluyendo el tipo de cálculo (suma, promedio, conteo, etc.) y el formato de los números. <resumen>La configuración de campos de valor permite cambiar la función de resumen aplicada (SUMA, PROMEDIO, CONTAR, MAX, MIN, PRODUCTO, etc.) y el tipo de cálculo mostrado (% del total general, % del total de fila/columna, diferencia respecto a, etc.). También se puede aplicar formato de número al campo.</resumen>`, files: [] },
                    { title: 'Subtotales', comment: `Las preguntas de este tema evalúan el uso de subtotales en tablas dinámicas para mostrar totales parciales por grupo, y la función SUBTOTALES en hojas de cálculo. <resumen>Los subtotales en tablas dinámicas pueden mostrarse en la parte superior o inferior de cada grupo de elementos, o desactivarse. La función SUBTOTALES en una hoja de cálculo realiza cálculos (suma, promedio, conteo, etc.) considerando solo las filas visibles, útil cuando se trabaja con filtros aplicados.</resumen>`, files: [] },
                    { title: 'Gráficos dinámicos', comment: `Las preguntas de este tema evalúan la creación y personalización de gráficos dinámicos vinculados a tablas dinámicas, que se actualizan automáticamente al cambiar los datos o los filtros. <resumen>Un gráfico dinámico está vinculado a una tabla dinámica y se actualiza automáticamente al cambiar sus datos o filtros. Se puede insertar desde la pestaña Analizar o Insertar. Soporta los mismos tipos de gráfico que los gráficos estáticos (barras, líneas, circular, etc.) y permite usar segmentaciones de datos para filtrar interactivamente.</resumen>`, files: [] }
                ]
            },

            {
                value: 'funciones_avanzadas', label: 'Funciones Avanzadas',
                subtopics: [
                    { title: 'Análisis de datos: estadística descriptiva e histograma', comment: `Las preguntas de este tema evalúan el uso de las herramientas del complemento Análisis de datos para calcular estadística descriptiva y generar histogramas de frecuencia. <resumen>El complemento Análisis de datos de Excel incluye la herramienta Estadística descriptiva, que calcula automáticamente media, mediana, moda, desviación estándar, varianza, rango, mínimo, máximo, entre otros. La herramienta Histograma genera tablas de frecuencia y gráficos de distribución a partir de un rango de datos y un rango de clases (bin).</resumen>`, files: [] },
                    { title: 'Funciones financieras', comment: `El contenido de las preguntas de este tema aplica las siguientes funciones de Microsoft Excel: <resumen>VA (Valor Actual), VF (Valor Futuro), NPER (Número de Períodos), PAGO (Pago periódico total), PAGOINT (Pago de intereses de un período), PAGOPRIN (Pago al capital de un período), TASA (Tasa de interés por período), TIR (Tasa Interna de Retorno), VNA (Valor Neto Actual). Estas funciones permiten resolver problemas de matemática financiera como anualidades, préstamos y evaluación de inversiones.</resumen>`, files: [] }
                ]
            },

            {
                value: 'automatizacion_manejo_datos', label: 'Automatización de Tareas y Manejo de Datos',
                subtopics: [
                    { title: 'Macros', comment: `Las preguntas de este tema evalúan la grabación, edición y ejecución de macros en Excel para automatizar tareas repetitivas, incluyendo conceptos básicos de VBA. <resumen>Una macro es una secuencia de instrucciones grabadas o escritas en VBA (Visual Basic for Applications) que automatiza tareas repetitivas en Excel. Se pueden grabar con la grabadora de macros, editar en el Editor de VBA, guardar en el libro actual o en el libro personal de macros, y ejecutar mediante un acceso directo, botón o desde la pestaña Programador/Desarrollador.</resumen>`, files: [] },
                    { title: 'Asignación de macros a controles', comment: `Las preguntas de este tema evalúan la inserción macros en objetos no están enfocadas a la programación ni crear botones. <resumen>Excel permite insertar a figuras las macros, no se realizará programación de macros.</resumen>`, files: [] },
                    { title: 'Funciones de búsqueda y referencia', comment: `El contenido de las preguntas de este tema aplica las siguientes funciones de Microsoft Excel: <resumen>BUSCAR (busca un valor en un rango de una fila o columna), BUSCARH (busca un valor en la primera fila de una tabla y devuelve el valor en la misma columna de una fila especificada), BUSCARV (busca un valor en la primera columna de una tabla y devuelve el valor en la misma fila de una columna especificada). Son funciones esenciales para cruzar y recuperar datos entre tablas.</resumen>`, files: [] },
                    { title: 'Funciones de totales condicionales', comment: `El contenido de las preguntas de este tema aplica las siguientes funciones de Microsoft Excel: <resumen>CONTAR.SI (cuenta celdas que cumplen un criterio), CONTAR.SI.CONJUNTO (cuenta celdas que cumplen múltiples criterios), SUMAR.SI (suma celdas que cumplen un criterio), SUMAR.SI.CONJUNTO (suma celdas que cumplen múltiples criterios). Estas funciones permiten realizar cálculos selectivos sobre rangos de datos según condiciones definidas.</resumen>`, files: [] },
                    { title: 'Funciones de base de datos', comment: `El contenido de las preguntas de este tema aplica las siguientes funciones de Microsoft Excel: <resumen>BDCONTARA (cuenta celdas no vacías que cumplen criterios), BDCONTAR (cuenta celdas con números que cumplen criterios), BDMAX (devuelve el máximo de un campo según criterios), BDMIN (devuelve el mínimo de un campo según criterios), BDSUMA (suma los valores de un campo según criterios), BDPROMEDIO (calcula el promedio de un campo según criterios), BDPRODUCTO (multiplica los valores de un campo según criterios). Todas trabajan con una estructura de base de datos (encabezados, datos, criterios).</resumen>`, files: [] }
                ]
            },

            {
                value: 'ofimatica_nube', label: 'Ofimática en la Nube',
                subtopics: [
                    { title: 'Office 365', comment: `Las preguntas de este tema evalúan el conocimiento de las aplicaciones y servicios incluidos en Microsoft 365 (antes Office 365), incluyendo almacenamiento en OneDrive, colaboración en tiempo real y las versiones web de Word, Excel y PowerPoint. <resumen>Microsoft 365 es la suite de productividad de Microsoft en la nube. Incluye versiones de escritorio y web de Word, Excel, PowerPoint, Outlook, Teams, OneNote, entre otras. Ofrece almacenamiento en OneDrive (1TB+), coautoría en tiempo real, sincronización entre dispositivos y suscripción mensual o anual.</resumen>`, files: [] },
                    { title: 'Google Drive', comment: `Las preguntas de este tema evalúan el uso de Google Drive como plataforma de almacenamiento en la nube y suite ofimática, incluyendo Google Docs, Sheets y Slides para la creación y colaboración de documentos. <resumen>Google Drive es el servicio de almacenamiento en la nube de Google. Integra las aplicaciones Google Docs (procesador de texto), Google Sheets (hoja de cálculo) y Google Slides (presentaciones), todas con colaboración en tiempo real y sin necesidad de instalar software. Los archivos se guardan automáticamente y son accesibles desde cualquier dispositivo.</resumen>`, files: [] },
                    { title: 'Dropbox', comment: `Las preguntas de este tema evalúan el uso de Dropbox como servicio de almacenamiento y sincronización de archivos en la nube, incluyendo el uso compartido de archivos y carpetas. <resumen>Dropbox es un servicio de almacenamiento en la nube que permite sincronizar archivos entre múltiples dispositivos y compartirlos con otros usuarios. Ofrece una carpeta local que se sincroniza automáticamente con la nube. Incluye funciones de historial de versiones, recuperación de archivos eliminados y colaboración mediante carpetas compartidas.</resumen>`, files: [] }
                ]
            }
        ]
    }
